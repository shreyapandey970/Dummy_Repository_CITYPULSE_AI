...
const getIconHtml = (issueType: IssueType) => {
  ...
  switch (issueType) {
    ...
    case "fallen_tree":
      ...
      break;
    default:
      icon = <div style={{ ...commonStyle, fontSize: "18px", textAlign: "center" }}>?</div>;
  }
  ...
  useEffect(() => {
    const validComplaintPoints = complaints
      .map((c) => {
        ...
        const [lat, lng] = parts.map((p) => parseFloat(p));
...
export function MapView({ complaints, route }: { complaints: Complaint[]; route: LatLngExpression[] }) {
  ...
  // Circle logic (covers the route area instead of straight line)
  ...
  if (route.length > 1) {
    ...
    routeCircle = { center: route[0], radius: 5000 }; // default 5km radius
  }
